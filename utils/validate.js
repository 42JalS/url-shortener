exports.isValidCustomUrl = customUrl => {
	if (customUrl === '' || !customUrl) return false;
	const regex = /\p{P}|\p{C}|\p{Z}|\s/gu;
	return !regex.test(url.replace(/[-_]/g, ''));
}

exports.isValidCustomUrlisValidOriginalUrl = originalUrl => {
	if (originalUrl === '' || !originalUrl) return false;
